...
/**
 * Profile "local": sobe um PostgreSQL 16 embarcado (zonky embedded-postgres),
 * executa as migrations Flyway normalmente e expoe o DataSource primario.
 *
 * Este bean só é criado quando cvfacil.local.use-embedded-db=true (padrão).
 * Para usar o banco de dados Neon (nuvem), defina em application-local.yml:
 *
 *   cvfacil:
 *     local:
 ...
 *       password: <senha>
 *
 * Quando use-embedded-db=false, o Spring Boot Autoconfigure cria o DataSource
 * a partir do spring.datasource.* configurado acima — este bean é ignorado.
 *
 * Produção (Neon via variáveis de ambiente) NÃO é afetada — este bean só existe
 * quando o profile "local" está ativo.
 */
@Configuration
@Profile("local")
