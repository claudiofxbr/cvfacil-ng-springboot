...
public interface UserRepository extends JpaRepository<User, UUID> {
  ...
  /**
   * Decremento atômico de 1 crédito — UPDATE ... WHERE credits >= 1 é atômico a nível de linha no
   * Postgres, sem precisar de lock explícito (mesmo princípio usado para corrigir a race condition
   * do audit log). Retorna 0 linhas afetadas quando o saldo é insuficiente (nunca fica negativo).
   */
  @Modifying
