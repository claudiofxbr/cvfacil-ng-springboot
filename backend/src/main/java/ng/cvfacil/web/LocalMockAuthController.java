 ...
 *
 * <p>Simula o fluxo de autenticação Google sem precisar de credenciais OAuth reais. Útil em
 * desenvolvimento para testar login social sem acesso ao Google Cloud Console.
 *
 * <p>Fluxo: 1. Frontend clica em "Entrar com Google". 2. Backend intercepta
 * /oauth2/authorization/google (credenciais dummy) e redireciona para
 * http://localhost:3000/login?mock_oauth=true. 3. Frontend detecta o param e exibe um formulário
 * simples pedindo o e-mail. 4. Usuário informa um e-mail qualquer e clica em "Continuar". 5.
 * Frontend chama POST /api/auth/mock-google com o e-mail. 6. Este controller cria o usuário (se
 * ainda não existir) e emite tokens normais.
 */
@RestController
...
public class LocalMockAuthController {
  ...
  public ResponseEntity<LoginResponse> mockGoogle(
      @Valid @RequestBody MockGoogleRequest req, HttpServletRequest http) {
        ...
        users
            ...
            .orElseGet(
                () -> {
                  ...
                  // displayName extraído da parte local do e-mail (ex: "joao.silva@..." →
                  // "joao.silva")
    ...
    audit.record(
        u.getId(), "MOCK_GOOGLE_LOGIN", http.getRemoteAddr(), http.getHeader("User-Agent"), null);
